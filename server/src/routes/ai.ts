...
router.post('/fill', async (req, res) => {
  ...
  console.log('DEBUG OPENAI_API_KEY length:', apiKey?.length ?? 'undefined')
  if (!apiKey) {
    res.status(500).json({
      message: 'OpenAI API key is not configured on the server.',
      debug: `length=${apiKey?.length ?? 'undefined'}`,
    })
    return
  }
  ...
  try {
    ...
    if (!deckle || !rolls || !Array.isArray(rolls) || rolls.length === 0) {
      res.status(400).json({ message: 'Invalid payload: deckle and rolls are required.' })
      return
    }
    ...
    if (!content) {
      res.status(502).json({ message: 'AI 응답이 비어 있습니다.' })
      return
    }
    ...
    try {
      ...
      res.status(502).json({ message: 'AI 응답을 JSON으로 파싱할 수 없습니다.' })
      return
    }
